#!/usr/bin/env tsx
/**
 * Drift detection for New Zealand Law MCP.
 *
 * Checks if upstream legislation.govt.nz content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('New Zealand Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'NewZealand-Law-MCP/1.0 drift-detect' },
      });

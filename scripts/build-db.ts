#!/usr/bin/env tsx
/**
 * Database builder for New Zealand Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building New Zealand Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from New Zealand statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'NZ');
    insertMeta.run('source', 'legislation.govt.nz');
    insertMeta.run('licence', 'Creative Commons Attribution 4.0 International (CC BY 4.0)');
  });
  writeMeta();

#!/usr/bin/env tsx
/**
 * New Zealand Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.legislation.govt.nz';
const PORTAL_NAME = 'New Zealand Legislation';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/newzealand-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('New Zealand Law MCP — Data Freshness Check');

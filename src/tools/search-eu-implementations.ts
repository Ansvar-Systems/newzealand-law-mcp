/**
 * search_eu_implementations — Search EU directives/regulations with NZ implementing legislation.
 */
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as nz_statute_count
  ...
  if (input.has_nz_implementation) {
    sql += ' HAVING nz_statute_count > 0';
  }

/**
 * validate_eu_compliance — Check EU/international alignment status for a NZ statute.
 */
...
export async function validateEUCompliance(
  ...
  if (euRefCount === 0) {
    return {
      results: {
        ...
        eu_references_found: 0,
        warnings: [],
        recommendations: ['No EU/international cross-references found for this statute. This may be a purely domestic New Zealand law.'],
      },
      _metadata: generateResponseMetadata(db),
    };
  }
  ...
  if (completeCount > 0 && partialCount === 0 && unknownCount === 0) {
    ...
    warnings.push(`${partialCount} EU/international reference(s) have partial alignment status.`);
  } else {
    compliance_status = 'unclear';
    if (unknownCount > 0) {
      recommendations.push(`${unknownCount} EU/international reference(s) have unknown alignment status. Manual review recommended.`);
    }
  }

/**
 * get_nz_implementations — Find NZ statutes implementing a specific EU directive/regulation.
 *
 * New Zealand is not an EU member but many NZ laws align with EU/international frameworks
 * (e.g., Privacy Act 2020 with GDPR, Commerce Act 1986 with EU competition law).
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetNzImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface NzImplementationResult {
  document_id: string;
...
export async function getNzImplementations(
  db: InstanceType<typeof Database>,
  input: GetNzImplementationsInput,
): Promise<ToolResponse<NzImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as NzImplementationResult[];
